...
static apr_status_t socket_cleanup(void *sock)
{
    ...
    if (thesocket->bound && thesocket->local_addr->family == APR_UNIX) {
        ...
        /* XXX: zhaoyongsheng delete it, because when start logrotator, the socket file will deleted */
...
{
    /* XXX: zhaoyongsheng add it, we delete socket file before call bind() to avoid bind fail */
#if APR_HAVE_SOCKADDR_UN
    if (sa->family == APR_UNIX) {
        unlink(sa->sa.unx.sun_path);
    }
#endif
